...
function onlyPurchasableItems(list){
	return list.filter(item=>item.gold.purchasable);
}
...
function getItems() {
	...
	var params = ["itemListData=colloq,inStore,effect,gold,image,maps,requiredChampion,sanitizedDescription,tags,tree,from"];
	if(!cache.items) {
		cache.items = makeRiotRequest(path, params)
			.then(onlySummonersRiftNew)
			.then(onlyPurchasableItems);	
	}
	return cache.items;
}
